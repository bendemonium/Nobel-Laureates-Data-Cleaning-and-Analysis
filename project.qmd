...
```{r}
set.seed(123)  
split <- initial_split(counts, prop = 0.8, strata = "Category")
...
```{r}
glm_fit <- glm_wf |> fit(data = train_data)
tree_fit <- tree_wf |> fit(data = train_data)
```

```{r}
glm_pred <- glm_fit |> predict(new_data = test_data)
tree_pred <- tree_fit |> predict(new_data = test_data)
```
